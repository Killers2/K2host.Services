...
namespace K2host.Services.Ldap.Classes
{
    ...
    public class LdapPacket : LdapAttribute
    {
        ...
        public LdapPacket(int messageId)
        ...
        public static LdapPacket ParsePacket(byte[] bytes)
        {
            ...
            var contentLength = bytes.BerLengthToInt(1, out var lengthBytesCount);
        ...
        public static bool TryParsePacket(Stream stream, out LdapPacket packet)
        {
            try
            {
                ...
                if (i != 0)
                {
                    var contentLength = stream.BerLengthToInt(out int n);
